...
def transform_hits(df):
    return SearchKeywordGlueJob(None, "", "s3://placeholder/output").transform_hits(df)


def publish_single_output_file(output_path, stage_dir):
    job = SearchKeywordGlueJob(None, "", output_path.rsplit("/", 1)[0])
    job.output_path = output_path
    return job.publish_single_output_file(stage_dir)
